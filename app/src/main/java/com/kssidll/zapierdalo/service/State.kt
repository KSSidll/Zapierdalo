...
/**
 * Helper function to remove the need to define the [Context.getSharedPreferences] mode
 * as the only non deprecated mode is [Context.MODE_PRIVATE]
...
fun Context.getServiceState(serviceName: String): ServiceState {
    val value = getSharedPreferences(serviceName).getString(
        serviceNameToStateKey(serviceName),
        ServiceState.STOPPED.name
    )!! // it's impossible for this to be null, but kotlin LSP thinks otherwise? i'm confused
 ...
 *
 * Checks the system instead of the shared preferences so it can't be listened on
 * and isn't efficient, it also relies on a deprecated method that exists only for compatibility
