...
@Composable
fun RunActionScreen(
    uiState: RunActionUiState,
    onEvent: (event: RunActionEvent) -> Unit,
    ...
    Scaffold(
        bottomBar = {
            SecondaryAppBar(
                title = {},
                onBack = {
                    onEvent(RunActionEvent.NavigateBack)
                },
                actions = {
                    IconButton(
                        onClick = {
                            onEvent(RunActionEvent.Delete)
                        }
                    ) {
                        Icon(
        ...
        Surface(
            ...
            AnimatedVisibility(
                visible = uiState.runAction != null,
                enter = fadeIn(),
                exit = fadeOut()
            ) {
                AndroidView(
